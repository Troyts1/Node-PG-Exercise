...
let router = new express.Router();
  ...
  router.put("/:id", async function(req, res, next) {
    try {
      ...
      let currBal = await db.query(
        `SELECT paid_date FROM invoices
         WHERE id = $1`,
        [id]
      );
  
      if (currBal.rows.length === 0) {
        throw new ExpressError(`No invoice found with id: ${id}`, 404);
      }
  
      const currPayDate = currBal.rows[0].paid_date;
  ...
  router.delete("/:id", async function(req, res, next) {
    try {
      ...
      if (result.rows.length === 0) {
        throw new ExpressError(`No company found with the id of ${id}`, 404);
      }
